...
exports.requeryBettingController = async (req, res) => {
  try {
    ...
    console.log("[BETTING REQUERY][REQUEST]", url);

    const axiosResponse = await axios.get(url);
    const raw = axiosResponse.data;

    console.log("[BETTING REQUERY][RESPONSE]", raw);
    ...
    // FAILED → Refund full amount including fee
    if (status === "ORDER_FAILED" || status === "FAILED") {
      txn.status = "failed";

      const refundAmount = Number(
        txn.totalDebited || (txn.amount + (txn.fee || 0))
      );
      ...
      return res.json({
        status: false,
        message: "Betting transaction failed. Refunded.",
    ...
    console.error("Betting requery error:", err);
