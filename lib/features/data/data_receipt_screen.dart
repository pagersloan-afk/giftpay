...
class DataReceiptScreen extends StatelessWidget {
  ...
  const DataReceiptScreen({super.key, required this.txn});
  ...
  Widget build(BuildContext context) {
    ...
    final network = txn["network"] ?? "Unknown Network";
    final phone = txn["phone"] ?? txn["phoneNumber"] ?? _extractPhone(title);
    final amount = _formatAmount(txn["amount"]);
    final plan = txn["plan"] ?? txn["planName"] ?? "Data Plan";
    final date = _formatDate(txn["timestamp"] ?? txn["date"]);
    final id = txn["id"] ?? "N/A";
    final message = txn["message"] ?? "Data purchase completed successfully.";
